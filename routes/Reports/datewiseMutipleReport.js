...
router.get('/multiple', async (req, res) => {
    ...
    if (!deviceCodes || !date) {
        return res.status(400).json({ error: 'Device codes and date are required.' });
    }
    ...
    try {
        ...
        // Ensure COW, BUF, TOTAL are present
        ...
        if (totals.length === 0 && records.length === 0) {
            return res.status(404).json({ error: 'No records found for the given device codes and date.' });
        }
