...
router.post("/", upload.single("file"), async (req, res) => {
  ...
  const clrBufEffectiveDate = req.body?.clrBufEffectiveDate;
  if (!clrBufEffectiveDate) {
    deleteFile(filePath);
    return res
      .status(400)
      .json({ error: "Missing clrBufEffectiveDate in request body" });
  }

  const date = new Date(clrBufEffectiveDate);
  ...
  fs.createReadStream(filePath)
    ...
    .on("end", async () => {
      try {
        ...
        const clrBufId = Math.floor(1000 + Math.random() * 9000); // 4-digit random

        const updateMain = await model.updateOne(filter, {
          $set: {
            clrBufTable: sortedJson,
          },
        });
        ...
        if (isDevice) {
          await deviceModel.updateOne(filter, {
            $set: {
              "rateChartIds.clrBufId": clrBufId,
              "effectiveDates.clrBufEffectiveDate": formattedDate,
              "isDeviceRateTable.clrBufTable": true,
            },
          });
        } else {
          await deviceModel.updateMany(
            { dairyCode: id },
            {
              $set: {
                "rateChartIds.clrBufId": clrBufId,
                "effectiveDates.clrBufEffectiveDate": formattedDate,
                "isDeviceRateTable.clrBufTable": false,
              },
            }
          );
        }
        ...
        res.status(200).json({
          message: `Uploaded clr Buf Table to ${isDevice ? "device" : "dairy"} ${id}`,
